"""
adset_atribute — technical attributes of each ad set, written to property_adsets.

Targets ad sets that changed (per `actions`, object_type=CAMPAIGN, excluding
audience-share events) or appear in `insights` with no property row yet, skipping
tombstoned objects. A 404-style Meta error tombstones the object instead.

Standalone, .env-driven port of adset_atribute.ipynb.
...
object_type = "adset"
table = "property_adsets"

exclude_list = [
    "__module__", "__doc__", "daily_imps", "frequency_cap", "frequency_cap_reset_period",
    ...
    "time_start", "rb_prediction_id", "execution_options", "tune_for_category",
    "date_format", "campaign_spec", "budget_schedule_specs", "budget_source",
    "budget_split_set_id", "is_sac_cfca_terms_certified",
]
...
def main():
    print(f"[START] adset_atribute\t{datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')}", flush=True)
    ...
    fields = list(set(AdSet.Field.__dict__.keys()) - set(exclude_list))

    cur.execute(SELECT_TARGETS)
    rows = cur.fetchall()
    allowed = set(account_ids)
    adsets = {row[0]: row[2] for row in rows if row[1] in allowed}
    ...
    for adset_id in tqdm(adsets.keys()):
        try:
            ...
            adset_data = modify_object_data(adset_data)
            cur, connection = reconnection_to_database(host, port, database, user, password, connection)
            if adsets[adset_id]:
                store_object(cur, connection, table, adset_data)
            else:
                delete_object(cur, table, adset_id)
                store_object(cur, connection, table, adset_data)
            ...
            if code == 100 and subcode in (33, 1487221):
                cur, connection = reconnection_to_database(host, port, database, user, password, connection)
                store_deleted_object(cur, connection, adset_id, object_type, accounts[adset_id], "deleted_objects")
            else:
                print(f"[WARN] FacebookRequestError for adset {adset_id}: code={code}", flush=True)

    print(f"[END] adset_atribute\t{datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')}", flush=True)
