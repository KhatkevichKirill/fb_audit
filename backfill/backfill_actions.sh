#!/bin/bash
# Backfill actions (ad change history) in N-day batches.
# Usage: ./backfill_actions.sh [START_DATE] [END_DATE]
...
echo "================================================"
echo " Actions backfill: $START -> $END (${BATCH_DAYS}-day batches)"
echo "================================================"
...
while [[ "$current" < "$END" || "$current" == "$END" ]]; do
    ...
    LOG="$LOG_DIR/backfill_actions_batch${batch}_${current}_${batch_end}.log"
    echo ""
    echo "[Batch $batch] $current -> $batch_end"
    echo "  Log: $LOG"

    ACTIONS_START_DATE="$current" ACTIONS_END_DATE="$batch_end" \
